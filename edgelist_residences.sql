WITH residences_facebook_users AS (
	WITH residences_facebook_groups AS (
		SELECT fg.group_id, fg.name, sg.name, sgg.child_id FROM supergroupsgroups sgg
			...
			WHERE sgg.parent_id = 2)
	-- parent_id 2 = residences
	SELECT fu.uid, fu.name FROM facebook_users fu
		...
		WHERE fg.group_id IN (SELECT group_id FROM residences_facebook_groups))
SELECT uid1 AS source, uid2 AS target
	...
	WHERE fu1.nu=1 AND fu2.nu=1 AND (uid1 IN (SELECT uid FROM residences_facebook_users) AND uid2 IN (SELECT uid FROM residences_facebook_users));